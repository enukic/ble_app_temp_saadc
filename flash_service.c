...
static void on_write(ble_fs_t *p_service, ble_evt_t const * p_ble_evt)
{
//    NRF_LOG_INFO("on_write: Called");
    ble_gatts_evt_write_t const * p_evt_write = &p_ble_evt->evt.gatts_evt.params.write;
    os_evt event;

    if((p_evt_write->handle == p_service->char_cnt_handle.value_handle))
    {
...
static uint32_t char_flash_temp_add(ble_fs_t * p_our_service)
{
    uint32_t            err_code;
    ble_uuid_t          char_uuid;
    ble_uuid128_t       base_uuid = BLE_UUID_OUR_BASE_UUID;
